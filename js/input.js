/* **********************************************
     Begin taxonomy_relationship.js
********************************************** */
...
(function($){
	...
	*/
	...
	acf.fields.taxonomy_relationship = {
		...
		add : function( $a ){
			...
			{
				alert( acf.l10n.taxonomy_relationship.max.replace('{max}', this.o.max) );
				return false;
			}
			...
			var data = {
				...
				tmpl = _.template(acf.l10n.taxonomy_relationship.tmpl_li, data);
	...
	*/
	...
	$(document).on('change', '.acf_taxonomy_relationship .select-post_type', function(e){
		
		// vars
		var val = $(this).val(),
			$el = $(this).closest('.acf_taxonomy_relationship');
	    ...
	    // fetch
	    acf.fields.taxonomy_relationship.set({ $el : $el }).fetch();
		
	});

	
	$(document).on('click', '.acf_taxonomy_relationship .relationship_left .relationship_list a', function( e ){
		
		e.preventDefault();
		
		acf.fields.taxonomy_relationship.set({ $el : $(this).closest('.acf_taxonomy_relationship') }).add( $(this) );
		
		$(this).blur();
		
	});
	
	$(document).on('click', '.acf_taxonomy_relationship .relationship_right .relationship_list a', function( e ){
		
		e.preventDefault();
		
		acf.fields.taxonomy_relationship.set({ $el : $(this).closest('.acf_taxonomy_relationship') }).remove( $(this) );
		
		$(this).blur();
		
	});
	
	$(document).on('keyup', '.acf_taxonomy_relationship input.relationship_search', function( e ){
	    ...
	    // fetch
	    clearTimeout( acf.fields.taxonomy_relationship.timeout );
	    acf.fields.taxonomy_relationship.timeout = setTimeout(function(){
	...
	$(document).on('keypress', '.acf_taxonomy_relationship input.relationship_search', function( e ){
